...
## Marigold/CrossPoint layout

`partitions.csv` defines the layout used by Marigold full-flash images and the
CrossPoint firmware family:
...
- **`firmware.bin`** — app image for `ota_0`. Flash to `0x10000`. Updates the
  ...
  flasher, `esptool write_flash 0x10000`, and in-app updater consume. The web
  installer explicitly forbids whole-chip erase for these app-only manifests;
  CI verifies that contract before Pages deployment.
...
same X4 app image under Marigold's in-app updater trigger name.
...
7.44 MB slots) from the Marigold/CrossPoint layout (`ota_1` at `0x650000`, with
6.25 MB slots). It uses a smaller 792×528 UC8253 panel with a BQ27220 battery
gauge instead of the X4's ADC divider. Support lives behind the
...
updater), **`FWUPDX3.BIN`** (Marigold's X3 one-shot trigger), and
...
source, reads the installed partition table, validates the complete ESP
application image against the discovered inactive slot, writes that slot, and
switches `otadata`. Reading the table is required on app-only X3 installations,
where the stock and Marigold/CrossPoint `ota_1` offsets differ. A failed or
completed attempt clears only the pending-selection record.
...
- [x] Marigold/CrossPoint dual-OTA partition table (`partitions.csv`) plus
...
- [x] **Boot-time SD updater** (`fw::ota_update`) — on boot, `/FWUPDATE.BIN` is
      ...
      from the installed partition table, selected via `otadata`, deleted, and

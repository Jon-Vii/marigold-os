...
impl<T, const N: usize> ChannelState<T, N> {
    ...
    where
        T: Clone,
    {
        let (result, waker) = self.try_peek_with_context_defer_wake(cx);
    ...
    fn try_peek_with_context_defer_wake(&mut self, cx: Option<&mut Context<'_>>) -> (Result<T, TryReceiveError>, Option<Waker>)
    where
        T: Clone,
    {
        let waker = self.take_senders_waker_if_full();

        if let Some(message) = self.queue.front() {
            (Ok(message.clone()), waker)
        } else {
            if let Some(cx) = cx {
                self.receiver_waker.register(cx.waker());
            }
            (Err(TryReceiveError::Empty), waker)
        }
    }

    fn try_receive_with_context(&mut self, cx: Option<&mut Context<'_>>) -> Result<T, TryReceiveError> {
        let (result, waker) = self.try_receive_with_context_defer_wake(cx);
        if let Some(waker) = waker {
            waker.wake();
        }
        result
    ...
    fn try_receive_with_context_defer_wake(&mut self, cx: Option<&mut Context<'_>>) -> (Result<T, TryReceiveError>, Option<Waker>) {
        let waker = self.take_senders_waker_if_full();
    ...
    fn poll_receive(&mut self, cx: &mut Context<'_>) -> Poll<T> {
        let (result, waker) = self.poll_receive_defer_wake(cx);
        if let Some(waker) = waker {
            waker.wake();
        }
        result
    ...
    fn poll_receive_defer_wake(&mut self, cx: &mut Context<'_>) -> (Poll<T>, Option<Waker>) {
        let waker = self.take_senders_waker_if_full();

        if let Some(message) = self.queue.pop_front() {
            (Poll::Ready(message), waker)
        } else {
            self.receiver_waker.register(cx.waker());
            (Poll::Pending, waker)
        }
    ...
    fn try_send_with_context(&mut self, message: T, cx: Option<&mut Context<'_>>) -> Result<(), TrySendError<T>> {
        let (result, waker) = self.try_send_with_context_defer_wake(message, cx);
        if let Some(waker) = waker {
            waker.wake();
        }
        result
    ...
    fn try_send_with_context_defer_wake(&mut self, message: T, cx: Option<&mut Context<'_>>) -> (Result<(), TrySendError<T>>, Option<Waker>) {
        match self.queue.push_back(message) {
            Ok(()) => {
                (Ok(()), self.receiver_waker.take_waker())
            }
            Err(message) => {
                if let Some(cx) = cx {
                    self.senders_waker.register(cx.waker());
                }
                (Err(TrySendError::Full(message)), None)
            }
        }
    }
    ...
    #[allow(dead_code)]
    fn clear(&mut self) {
        let ((), waker) = self.clear_defer_wake();
        if let Some(waker) = waker {
            waker.wake();
        }
    }

    fn clear_defer_wake(&mut self) -> ((), Option<Waker>) {
        let waker = self.take_senders_waker_if_full();
        self.queue.clear();
        ((), waker)
    }

    fn take_senders_waker_if_full(&mut self) -> Option<Waker> {
        if self.queue.is_full() {
            self.senders_waker.take_waker()
        } else {
            None
        }
    }
    ...
    M: RawMutex,
{
    ...
    fn lock_defer_wake<R>(&self, f: impl FnOnce(&mut ChannelState<T, N>) -> (R, Option<Waker>)) -> R {
        let (result, waker) = self.inner.lock(|rc| f(&mut *unwrap!(rc.try_borrow_mut())));
        if let Some(waker) = waker {
            waker.wake();
        }
        result
    ...
    fn try_receive_with_context(&self, cx: Option<&mut Context<'_>>) -> Result<T, TryReceiveError> {
        self.lock_defer_wake(|c| c.try_receive_with_context_defer_wake(cx))
    }
    ...
    where
        T: Clone,
    {
        self.lock_defer_wake(|c| c.try_peek_with_context_defer_wake(cx))
    }

    /// Poll the channel for the next message
    pub fn poll_receive(&self, cx: &mut Context<'_>) -> Poll<T> {
        self.lock_defer_wake(|c| c.poll_receive_defer_wake(cx))
    }

    fn try_send_with_context(&self, m: T, cx: Option<&mut Context<'_>>) -> Result<(), TrySendError<T>> {
        self.lock_defer_wake(|c| c.try_send_with_context_defer_wake(m, cx))
    }
    ...
    /// Clears all elements in the channel.
    pub fn clear(&self) {
        self.lock_defer_wake(|c| c.clear_defer_wake());
    }
